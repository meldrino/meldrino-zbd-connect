...
app.use('/zbdconnect', express.static('public'));
...
app.get('/start', async (req, res) => {
  ...
  ws.on('open', () => {
    ws.send(JSON.stringify({
      ...
      data: { browserOS: 'Windows', browserName: 'Chrome', QRCodeZClient: 'browser-extension' }
    }));
  });
  ...
  setTimeout(() => { delete sessions[sessionId]; }, 10 * 60 * 1000);
  res.json({ sessionId });
});
